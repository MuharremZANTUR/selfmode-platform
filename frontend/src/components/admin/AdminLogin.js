...
const AdminLogin = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend bağlantısı yoksa fallback mekanizması
      if (formData.email === 'admin@selfmode.app' && formData.password === 'admin123') {
        // Demo admin kullanıcısı için localStorage fallback
        const mockAdminData = {
          user: {
            id: 1,
            email: 'admin@selfmode.app',
            role: 'admin'
          },
        ...
        onLogin(mockAdminData.user, mockAdminData.token);
      } else {
        setError('Backend bağlantısı yok. Demo bilgileri: admin@selfmode.app / admin123');
      }
    } finally {
      setIsLoading(false);
    }
